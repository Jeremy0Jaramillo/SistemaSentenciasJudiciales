  ...
  export class EvaluacionComponent implements OnInit {
    ...
    // MODIFICADO: Obtener mensaje detallado de secciones incompletas - TODAS LAS SECCIONES
    getDetailedIncompleteSections(): string {
      const motivationType = this.evaluacionForm.get('motivationType')?.value
      let details: string[] = []
      ...
      if (!nonexistence?.lackFoundationNormative) {
        details.push('• Inexistencia: Complete fundamentación normativa')
      }
      if (!nonexistence?.reasonsNormative) {
        details.push('• Inexistencia: Ingrese razones normativas')
      }
      ...
      if (!nonexistence?.reasonsFactual) {
        details.push('• Inexistencia: Ingrese razones fácticas')
      }

      // Insuficiencia
      const insufficiency = this.evaluacionForm.get('insufficiency')?.value
      if (!insufficiency?.lackFoundationNormative) {
        details.push('• Insuficiencia: Complete fundamentación normativa')
      }
      if (!insufficiency?.reasonsNormative) {
        details.push('• Insuficiencia: Ingrese razones normativas')
      }
      ...
      if (!insufficiency?.reasonsFactual) {
        details.push('• Insuficiencia: Ingrese razones fácticas')
      }
      ...
      if (!appearance?.motivationalHabit) {
        ...
        // Verificar subsección específica
        const habitDetails = this.getAppearanceSubsectionDetails(appearance.motivationalHabit)
        details.push(...habitDetails)
      }
      ...
      if (this.areAllSectionsComplete()) {
        ...
        details.push('• Complete TODAS las secciones antes de acceder a la Evaluación Final')
      }
...
private isAppearanceComplete(): boolean {
  const appearance = this.evaluacionForm.get('appearance')?.value;

  // 1. Aún es necesario que el estudiante seleccione un vicio motivacional.
  if (!appearance?.motivationalHabit) {
    return false;
  }

  // 2. AHORA, VALIDAMOS TODAS LAS SUBSECCIONES OBLIGATORIAMENTE.
  // Se llama a cada función de validación individual.
  ...
  // La función solo devolverá 'true' si TODAS las subsecciones son válidas.
...
loadFormData() {
  this.firestore
    ...
    // NUEVO: Obtener detalles de subsecciones de Apariencia para el mensaje detallado
    private getAppearanceSubsectionDetails(habit: string): string[] {
      let details: string[] = []
      ...
      switch (habit) {
        case 'incoherence':
          ...
          if (!incoherence?.reasonsLogicaNormative) {
            details.push('• Incoherencia: Ingrese razones de lógica normativa')
          }
          if (!incoherence?.existsDecisionalNormative) {
            details.push('• Incoherencia: Complete incoherencia decisional normativa')
          }
          if (!incoherence?.reasonsDecisionalNormative) {
            details.push('• Incoherencia: Ingrese razones de decisional normativa')
          }
          if (!incoherence?.existsLogicalFactual) {
            details.push('• Incoherencia: Complete incoherencia lógica fáctica')
          }
          if (!incoherence?.reasonsLogicalFactual) {
            details.push('• Incoherencia: Ingrese razones de lógica fáctica')
          }
          if (!incoherence?.existsDecisionalFactual) {
            details.push('• Incoherencia: Complete incoherencia decisional fáctica')
          }
          if (!incoherence?.reasonsDecisionalFactual) {
            details.push('• Incoherencia: Ingrese razones de decisional fáctica')
          }
          break
        ...
        case 'inatinence':
          ...
          if (!inatinence?.reasonsInatinenceJuridical) {
            details.push('• Inatinencia: Ingrese razones jurídicas')
          }
          if (!inatinence?.existsInatinenceFactual) {
            details.push('• Inatinencia: Complete fundamentación fáctica')
          }
          if (!inatinence?.reasonsInatinenceFactual) {
            details.push('• Inatinencia: Ingrese razones fácticas')
          }
          break

        case 'incomprehensibility':
          const incomprehensibility = this.evaluacionForm.get('appearance.incomprehensibility')?.value
          if (!incomprehensibility?.existsIncomprehensibilityJuridical) {
            details.push('• Incomprensibilidad: Complete fundamentación jurídica')
          }
          if (!incomprehensibility?.reasonsIncomprehensibilityJuridical) {
            details.push('• Incomprensibilidad: Ingrese razones jurídicas')
          }
          if (!incomprehensibility?.existsIncomprehensibilityFactual) {
            details.push('• Incomprensibilidad: Complete fundamentación fáctica')
          }
          if (!incomprehensibility?.reasonsIncomprehensibilityFactual) {
            details.push('• Incomprensibilidad: Ingrese razones fácticas')
          }
          break

        case 'incongruity':
          const incongruity = this.evaluacionForm.get('appearance.incongruity')?.value
          if (!incongruity?.existsIncongruityNormativeParticipants) {
            details.push('• Incongruencia: Complete incongruencia normativa frente a partes')
          }
          if (!incongruity?.reasonsIncongruityNormativeParticipants) {
            details.push('• Incongruencia: Ingrese razones normativa partes')
          }
          if (!incongruity?.existsIncongruityNormativeLaw) {
            details.push('• Incongruencia: Complete incongruencia normativa frente al derecho')
          }
          if (!incongruity?.reasonsIncongruityNormativeLaw) {
            details.push('• Incongruencia: Ingrese razones normativa derecho')
          }
          if (!incongruity?.existsIncongruityFactualParticipants) {
            details.push('• Incongruencia: Complete incongruencia fáctica frente a partes')
          }
          if (!incongruity?.reasonsIncongruityFactualParticipants) {
            details.push('• Incongruencia: Ingrese razones fáctica partes')
          }
          if (!incongruity?.existsIncongruityFactualLaw) {
            details.push('• Incongruencia: Complete incongruencia fáctica frente al derecho')
          }
          if (!incongruity?.reasonsIncongruityFactualLaw) {
            details.push('• Incongruencia: Ingrese razones fáctica derecho')
          }
          break
      }
